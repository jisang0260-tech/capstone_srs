                                ...
                                const nas_if_t&         itf)
{
  ...
  // Force IMSI collection for every Attach Request before continuing with attach handling.
  return nas::handle_guti_attach_request_unknown_ue(enb_ue_s1ap_id, enb_sri, attach_req, pdn_con_req, args, itf);
}
...
bool nas::handle_identity_response(srsran::byte_buffer_t* nas_rx)
{
  ...
  if (id_resp.mobile_id.type_of_id != LIBLTE_MME_EPS_MOBILE_ID_TYPE_IMSI) {
    m_logger.error("Identity Response did not contain an IMSI.");
    return false;
  }
  ...
  m_emm_ctx.imsi = imsi;
  m_logger.info("Identity Response -- IMSI: %015" PRIu64, m_emm_ctx.imsi);
  srsran::console("Identity Response -- IMSI: %015" PRIu64 "\n", m_emm_ctx.imsi);
  ...
  if (!pack_attach_reject(nas_tx.get(), LIBLTE_MME_EMM_CAUSE_EPS_SERVICES_NOT_ALLOWED)) {
    return false;
  }

  m_s1ap->send_downlink_nas_transport(
      m_ecm_ctx.enb_ue_s1ap_id, m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), m_ecm_ctx.enb_sri);
  m_logger.warning("Downlink NAS: Sent Attach Reject after Identity Response.");
  srsran::console("Downlink NAS: Sent Attach Reject after Identity Response\n");
